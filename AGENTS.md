...
## Проверки и качество
- Базовые тесты: `make test` (phpunit внутри контейнера). Точный аналог вне make — `cd src && php artisan test`.
- Форматирование PHP — `cd src && ./vendor/bin/pint`; держись PSR-12/Laravel-стиля.
- Frontend сборка — `cd src && npm run build` (или `make npm cmd="run build"`); Vite кладёт билды в `public/build`.
